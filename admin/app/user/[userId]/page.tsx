  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (res.status === 200) {
          const data = (await res.json()) as { user?: User; status?: number };
        ...
        {!loading && !error && user && (
          <div className="grid gap-6 rounded-2xl border border-amber-500/30 bg-white/5 p-6 sm:grid-cols-2">
            ...
            <div className="space-y-3">
              ...
              {user.bio ? (
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-amber-300">Bio</p>
                  <p className="text-sm text-amber-50/90">{user.bio}</p>
                </div>
              ) : null}
            </div>
          </div>
